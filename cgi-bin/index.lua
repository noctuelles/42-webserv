...
for name, value in string.gmatch(info .. '&', '(.-)%=(.-)%&')
do
	value = string.gsub(value , '%+', ' ')
	value = string.gsub(value , '%%(%x%x)', function(dpc) return string.char(tonumber(dpc,16)) end )
	params[name] = value
...
local pagetop = [[<html><head><title>A Web page in Lua</title></head>
...
if params["net"] ~= ""  and params["net"] ~= nil then
	local inval = tonumber(params["net"])
if inval == nil then
