...
/*   Updated: 2022/11/21 15:15:48 by plouvel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace AutoIndex
{
	File::File(const string& path, const string& name, time_t last_modif, bool is_a_dir, off_t size) :
		...
		last_modified(std::make_pair(last_modif, ft::utils::formatTimeToRFC822(std::gmtime(&last_modif)))),
	...
	Dir::Dir(const char* path) : ptr(::opendir(path))
	{
		if (!ptr)
		{
			if (errno == ENOTDIR || errno == ENOENT)
				throw (ft::http::RequestHandler::Exception(ft::http::BadRequest));
			else
				throw (ft::http::RequestHandler::Exception(ft::http::InternalServerError));
		}
	}
	...
	const string	generatePage(const vector<ft::VirtServ::RouteOptions>& routes, const ft::VirtServ::RouteOptions& curr_route,
